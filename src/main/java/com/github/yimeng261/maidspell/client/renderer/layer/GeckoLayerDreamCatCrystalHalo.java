...
public class GeckoLayerDreamCatCrystalHalo<T extends Mob, R extends IGeoEntityRenderer<T>> extends GeoLayerRenderer<T, R> {
    ...
    public GeckoLayerDreamCatCrystalHalo(R renderer) {
        super(renderer);
    }

    @Override
    public GeoLayerRenderer<T, R> copy(R renderer) {
        return new GeckoLayerDreamCatCrystalHalo<>(renderer);
    }
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T entity,
        ...
        if (maid == null || maid.isInvisible() || maid.isSleeping() || !MaidHaloRenderPriority.shouldRenderDreamHalo(maid)) {
            return;
        }
    ...
    private static ResourceLocation texture(String path) {
        return ResourceLocation.fromNamespaceAndPath(MaidSpellMod.MOD_ID, "textures/entity/maid/halo/" + path);
    }
}
