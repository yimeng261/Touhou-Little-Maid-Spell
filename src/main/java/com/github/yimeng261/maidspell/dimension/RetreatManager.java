...
public class RetreatManager {
    ...
    /**
     * 已成功生成隐世之境结构的维度集合（持久到服务器关闭）
     */
    private static final Set<ResourceKey<Level>> generatedDimensions = ConcurrentHashMap.newKeySet();

    /**
     * 正在尝试生成结构的维度集合（Mixin 预约槽位，generate() 成功后移入 generatedDimensions）。
     * 与 generatedDimensions 分离，避免 Mixin 和 generate() 使用同一标记导致双重标记冲突。
     */
    private static final Set<ResourceKey<Level>> pendingGenerations = ConcurrentHashMap.newKeySet();
    ...
    private static final ResourceKey<StructureSet> HIDDEN_RETREAT_SET_KEY =
            ResourceKey.create(Registries.STRUCTURE_SET,
                    new ResourceLocation(MaidSpellMod.MOD_ID, "hidden_retreat_set"));
    ...
    /**
     * [Mixin 调用] 原子性预约生成槽位。
     * 只有当维度既不在 pending 也不在 generated 时才成功（CAS 语义）。
     *
     * @return true = 成功预约，本次 tryGenerateStructure 可继续；false = 已预约或已生成，应拦截
     */
    public static boolean tryBeginGeneration(ResourceKey<Level> key) {
        if (generatedDimensions.contains(key)) return false;
        return pendingGenerations.add(key);
    }

    /**
     * [generate() 调用] 生成成功，将维度从 pending 迁移到 generated。
     */
    public static void completeGeneration(ResourceKey<Level> key) {
        generatedDimensions.add(key);
        pendingGenerations.remove(key);
    }

    /**
     * [generate() 调用] 生成失败，释放 pending 槽位，允许下一个候选区块重试。
     */
    public static void cancelGeneration(ResourceKey<Level> key) {
        pendingGenerations.remove(key);
    }
    ...
    /**
     * @deprecated 使用 {@link #cancelGeneration(ResourceKey)} 替代。
     * 保留仅为兼容现有调用点，将在后续版本移除。
     */
    @Deprecated
    ...
    /**
     * @deprecated 使用 {@link #tryBeginGeneration(ResourceKey)} + {@link #completeGeneration(ResourceKey)} 替代。
     * 保留仅供 PlayerRetreatManager 的重启恢复代码使用。
     */
    @Deprecated
