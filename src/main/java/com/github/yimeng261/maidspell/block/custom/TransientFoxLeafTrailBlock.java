...
public class TransientFoxLeafTrailBlock extends WaterlilyBlock {
    public static final MapCodec<TransientFoxLeafTrailBlock> CODEC =
            simpleCodec(props -> new TransientFoxLeafTrailBlock(10, 18, props));

    private static final long REFRESH_INTERVAL_TICKS = 5L;
    private final int minLifetimeTicks;
    private final int maxLifetimeTicks;

    public TransientFoxLeafTrailBlock(int minLifetimeTicks, int maxLifetimeTicks, BlockBehaviour.Properties properties) {
        super(properties);
    ...
    public static BlockBehaviour.Properties createProperties() {
        return BlockBehaviour.Properties.ofFullCopy(Blocks.LILY_PAD)
                .strength(0.0F)
                .noCollission()
                .noOcclusion()
                .noLootTable();
