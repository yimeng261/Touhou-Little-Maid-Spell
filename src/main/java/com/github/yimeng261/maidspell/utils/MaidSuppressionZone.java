 ...
 * 「这一片地方，女仆不许插手」。
 *
 * <p>压制区内女仆不攻击、不喂食、不支援。判定得有个单一入口，散在各处各写一遍
 * 必然漏掉一处 —— 这个类就是那个入口。
 *
 * <p><b>为什么是静态区域表而不是直接问登记者。</b>女仆的战斗任务、饰品逻辑
 * 属于本模组的核心部分，登记者可能来自可选模组。核心去 import 一个可选模组下的类，
 * 缺模组时就是 {@code NoClassDefFoundError}。所以反过来：登记者单方面往这儿登记一片区域，
 * 核心那边只认坐标，谁登记的不关心。
 *
