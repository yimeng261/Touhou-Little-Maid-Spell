...
public class SlashBladeProvider extends ISpellBookProvider<MaidSlashBladeData, ResourceLocation> {
    ...
    /** SA 穿插节奏：每 N 次攻击序列开启一次 SA 窗口。 */
    private static final int SLASH_ART_INTERVAL_TICKS = 4;
    /** 直接技能穿插节奏：每 N 次攻击序列开启一次 DS 窗口。 */
    ...
    public void stopCasting(EntityMaid maid) {
        ...
        if (isSpellBook(slashBlade)) {
            slashBlade.getCapability(ItemSlashBlade.BLADESTATE).ifPresent(state -> {
                ...
                state.setTargetEntityId((LivingEntity) null);
            });
        }
    ...
    /**
     * 以平A为主轴，在可用窗口中穿插直接技能和SA，避免每次都被SA蓄力占满。
     */
    ...
    private void triggerSlashArt(EntityMaid maid, ItemStack slashBlade) {
        ...
        slashBlade.getCapability(ItemSlashBlade.BLADESTATE).ifPresent(state -> state.setTargetEntityId(target));
    ...
    private void performSlashBladeComboAttack(EntityMaid maid, ItemStack slashBlade, LivingEntity target) {
        ...
        slashBlade.getCapability(ItemSlashBlade.BLADESTATE).ifPresent(state -> {
            ...
            maid.getCapability(ItemSlashBlade.INPUT_STATE).map(input -> {
                ...
                try {
                    return state.progressCombo(maid);
                } finally {
                    if (addedClick) {
