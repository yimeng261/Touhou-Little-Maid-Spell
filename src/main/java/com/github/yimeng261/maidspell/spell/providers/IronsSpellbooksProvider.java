...
public class IronsSpellbooksProvider extends ISpellBookProvider<MaidIronsSpellData, SpellData> {
    ...
    protected List<SpellData> collectSpellFromSingleSpellBook(ItemStack spellBook, EntityMaid maid) {
        ...
        return spellContainer.getActiveSpells().stream().map(SpellSlot::spellData).toList();
    }
    ...
    public void initiateCasting(EntityMaid maid) {
        ...
        boolean spellWhiteListWhitelisted = !spellWhiteList.isEmpty() && SpellWhiteListSpellManager.getStoredSpellIds(spellWhiteList).contains(selectedSpellId);
    ...
    private void invokeSpellOnCast(EntityMaid maid, MaidIronsSpellData data, SpellData spellData, CastSource castSource) {
        AbstractSpell spell = spellData.getSpell();
        setupSpellTargetData(maid, spell);
    ...
    private boolean tryStartMaidRecastSession(EntityMaid maid, MaidIronsSpellData data, SpellData spellData, CastSource castSource) {
        AbstractSpell spell = spellData.getSpell();
    ...
    private void processMaidRecastSession(EntityMaid maid, MaidIronsSpellData data) {
        ...
        SpellData spellData = session.getSpell();
        if (spellData == null || spellData.getSpell() == null) {
    ...
    private void completeCasting(EntityMaid maid) {
        ...
        AbstractSpell spell = data.getCurrentCastingSpell().getSpell();
        MagicData magicData = data.getMagicData();
        SpellData currentSpell = data.getCurrentCastingSpell();
