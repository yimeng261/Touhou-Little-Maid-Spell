...
public class BaubleStateManager {
    ...
    private static final Map<UUID, List<ItemStack>> MAID_BAUBLE_SNAPSHOTS = new ConcurrentHashMap<>();
    ...
    public static void updateAndCheckBaubleState(EntityMaid maid) {
        LOGGER.info("updateAndCheckBaubleState");
        ...
        MAID_BAUBLE_SNAPSHOTS.put(maidUUID, new ArrayList<>(currentBaubles));
    }

    private static List<ItemStack> getPreviousBaubles(UUID maidUUID) {
        return MAID_BAUBLE_SNAPSHOTS.computeIfAbsent(maidUUID, k -> new ArrayList<>());
    }
