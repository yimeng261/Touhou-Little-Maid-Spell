...
public class MaidSpellCreativeTab {
    ...
    public static final RegistryObject<CreativeModeTab> MAID_SPELL_TAB = 
        CREATIVE_MODE_TABS.register("maid_spell_tab", () -> CreativeModeTab.builder()
            ...
            .icon(() -> {
                // 如果法术强化核心存在，使用它作为图标，否则使用附魔书
                if (MaidSpellItems.SPELL_ENHANCEMENT_CORE != null) {
                    return new ItemStack(MaidSpellItems.SPELL_ENHANCEMENT_CORE.get());
                } else {
                    return new ItemStack(Items.ENCHANTED_BOOK);
                }
            })
            .displayItems((parameters, output) -> {
                // 只有在铁魔法模组存在且物品已注册时才添加
                if (MaidSpellItems.isIronsSpellbooksLoaded() && MaidSpellItems.SPELL_ENHANCEMENT_CORE != null) {
