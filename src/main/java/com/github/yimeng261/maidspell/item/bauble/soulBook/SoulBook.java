...
public class SoulBook extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        TooltipHelper.addShiftTooltip(tooltip,
            List.of(
                Component.translatable("item.maidspell.soul_book.desc1").withStyle(ChatFormatting.GRAY)
            ),
            List.of(
                Component.translatable("item.maidspell.soul_book.desc2",
                    String.format("%.0f", Config.soulBookDamageThresholdPercent * 100)).withStyle(ChatFormatting.BLUE),
                Component.translatable("item.maidspell.soul_book.desc3",
                    String.format("%.1f", Config.soulBookDamageIntervalThreshold / 20.0)).withStyle(ChatFormatting.GOLD),
                Component.translatable("item.maidspell.soul_book.desc4").withStyle(ChatFormatting.LIGHT_PURPLE)
            ));
    }
}
