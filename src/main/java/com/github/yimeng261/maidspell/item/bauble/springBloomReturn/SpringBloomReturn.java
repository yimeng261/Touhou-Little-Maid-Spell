...
public class SpringBloomReturn extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        TooltipHelper.addShiftTooltip(tooltip,
            List.of(
                Component.translatable("item.maidspell.spring_bloom_return.desc1").withStyle(ChatFormatting.GRAY)
            ),
            List.of(
                Component.translatable(
                    "item.maidspell.spring_bloom_return.desc2",
                    Integer.toString(Config.springBloomReturnMaxStacks),
                    String.format("%.0f", Config.springBloomReturnStackDurationTicks / 20.0)
                ).withStyle(ChatFormatting.LIGHT_PURPLE),
                Component.translatable(
                    "item.maidspell.spring_bloom_return.desc3",
                    String.format("%.1f", Config.springBloomReturnDamageThreshold),
                    String.format("%.0f", Config.springBloomReturnDamageThresholdRatio * 100)
                ).withStyle(ChatFormatting.AQUA),
                Component.translatable(
                    "item.maidspell.spring_bloom_return.desc4",
                    String.format("%.0f", Config.springBloomReturnHealRatio * 100),
                    String.format("%.0f", Config.springBloomReturnCooldownRefundRatio * 100)
